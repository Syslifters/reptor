...
class Sslyze(ToolBase):
    ...
    weak_ciphers = ['DHE-DSS-DES-CBC3-SHA',
                    'DHE-DSS-AES128-SHA',
                    ...
                    'SRP-AES-128-CBC-SHA',
                    'SRP-AES-256-CBC-SHA']
    insecure_ciphers = ['ADH-DES-CBC3-SHA',
                        'ADH-AES128-SHA',
                        'ADH-AES128-SHA256',
    ...
    def get_weak_ciphers(self, target):
        ...
        for protocol, protocol_data in target['commands_results'].items():
            if len(protocol_data.get('accepted_cipher_list', list())) > 0:
                ...
                weak_ciphers = list(
                    filter(None, [c['openssl_name'] if c['openssl_name'] in self.weak_ciphers else None for c in protocol_data['accepted_cipher_list']]))
                if weak_ciphers:
                    result_protocols[protocol]['weak_ciphers'] = weak_ciphers
                insecure_ciphers = list(
                    filter(None, [c['openssl_name'] if c['openssl_name'] in self.insecure_ciphers else None for c in protocol_data['accepted_cipher_list']]))
                if insecure_ciphers:
    ...
    def get_certinfo(self, target):
        ...
        result_certinfo['certificate_untrusted'] = list(filter(None, [store["trust_store"]["name"] if not store["is_certificate_trusted"] else None for store in certinfo.get(
            'path_validation_result_list') or list()]))
        return result_certinfo
    ...
    def get_vulnerabilities(self, target):
        ...
        result_vulnerabilities['robot'] = commands_results.get(
            'robot', dict()).get('robot_result_enum') in ['VULNERABLE_STRONG_ORACLE', 'VULNERABLE_WEAK_ORACLE']
        return result_vulnerabilities
    ...
    def get_misconfigurations(self, target):
        ...
        result_misconfigs['compression'] = commands_results.get(
            'compression', dict()).get('compression_name') is not None
        result_misconfigs['downgrade'] = commands_results.get(
            'fallback', dict()).get('supports_fallback_scsv', True) is not True
        result_misconfigs['client_renegotiation'] = commands_results.get(
            'reneg', dict()).get('accepts_client_renegotiation')
        result_misconfigs['no_secure_renegotiation'] = commands_results.get(
            'reneg', dict()).get('supports_secure_renegotiation', True) is not True
    ...
    def format(self):
        ...
        for target in self.parsed_input:
            # Heading
            formatted += f"# {target['hostname']}:{target['port']} ({target['ip_address']})\n\n"
            # Protocols
            formatted += "**Protocols**\n\n"
            ...
            if not target['certinfo']['certificate_untrusted']:
                formatted += " * <span style=\"color: green\">Certificate is trusted</span>\n"
            else:
                formatted += " * <span style=\"color: red\">Certificate untrusted by:</span>\n"
                for store in target['certinfo']['certificate_untrusted']:
                    formatted += f"    * {store}\n"
        
            formatted += f" * Certificate matches hostname: " \
                         f"<span style=\"color: " \
            ...
            formatted += f" * SHA1 in certificate chain: " \
                         f"<span style=\"color: " \
            ...
            if any([v for k, v in target['vulnerabilities'].items()]):
                ...
                vulns = list()
                vulns.append("Heartbleed") if target['vulnerabilities'].get('heartbleed') else True
                vulns.append("Robot Attack") if target['vulnerabilities'].get('robot') else True
                vulns.append(
                    "OpenSSL CCS (CVE-2014-0224)") if target['vulnerabilities'].get('openssl_ccs') else True
                formatted += ', '.join(vulns) + '\n\n'
            # Misconfigurations
            if any([v for k, v in target['misconfigurations'].items()]):
                formatted += "**Misconfigurations**: "
                misconfigs = list()
                misconfigs.append("Compression") if target['misconfigurations'].get('compression') else True
                misconfigs.append("Downgrade Attack (no SCSV fallback)") if target['misconfigurations'].get('downgrade') else True
                misconfigs.append("No Secure Renegotiation") if target['misconfigurations'].get('no_secure_renegotiation') else True
                misconfigs.append("Client Renegotiation") if target['misconfigurations'].get('accepts_client_renegotiation') else True
                formatted += ', '.join(misconfigs) + '\n\n'

            # Weak ciphers
            if any([v.get('weak_ciphers', list()) + v.get('insecure_ciphers', list()) for k, v in target['protocols'].items()]):
                formatted += "**Weak Cipher Suites**\n\n"
                if target['protocols'].get('sslv2'):
                    formatted += " * **SSLv2**\n"
                    for c in target['protocols']['sslv2'].get('insecure_ciphers', list()):
                        formatted += f"    * {c} (<span style=\"color: red\">insecure</span>)\n"
                    for c in target['protocols']['sslv2'].get('weak_ciphers', list()):
                        formatted += f"    * {c}\n"
                if target['protocols'].get('sslv3'):
                    formatted += " * **SSLv3**\n"
                    for c in target['protocols']['sslv3'].get('insecure_ciphers', list()):
                        formatted += f"    * {c} (<span style=\"color: red\">insecure</span>)\n"
                    for c in target['protocols']['sslv3'].get('weak_ciphers', list()):
                        formatted += f"    * {c}\n"
                if target['protocols'].get('tlsv1'):
                    formatted += " * **TLS 1.0**\n"
                    for c in target['protocols']['tlsv1'].get('insecure_ciphers', list()):
                        formatted += f"    * {c} (<span style=\"color: red\">insecure</span>)\n"
                    for c in target['protocols']['tlsv1'].get('weak_ciphers', list()):
                        formatted += f"    * {c}\n"
                if target['protocols'].get('tlsv1_1'):
                    formatted += " * **TLS 1.1**\n"
                    for c in target['protocols']['tlsv1_1'].get('insecure_ciphers', list()):
                        formatted += f"    * {c} (<span style=\"color: red\">insecure</span>)\n"
                    for c in target['protocols']['tlsv1_1'].get('weak_ciphers', list()):
                        formatted += f"    * {c}\n"
                if target['protocols'].get('tlsv1_2'):
                    formatted += " * **TLS 1.2**\n"
                    for c in target['protocols']['tlsv1_2'].get('insecure_ciphers', list()):
                        formatted += f"    * {c} (<span style=\"color: red\">insecure</span>)\n"
                    for c in target['protocols']['tlsv1_2'].get('weak_ciphers', list()):
                        formatted += f"    * {c}\n"
                if target['protocols'].get('tlsv1_3'):
                    formatted += " * **TLS 1.3**\n"
                    for c in target['protocols']['tlsv1_3'].get('insecure_ciphers', list()):
                        formatted += f"    * {c} (<span style=\"color: red\">insecure</span>)\n"
