...
class Translate(Base):
    ...
    DEEPL_FROM = ['BG', 'CS', 'DA', 'DE', 'EL', 'EN', 'ES', 'ET', 'FI', 'FR', 'HU', 'ID', 'IT', 'JA',
                  'KO', 'LT', 'LV', 'NB', 'NL', 'PL', 'PT', 'RO', 'RU', 'SK', 'SL', 'SV', 'TR', 'UK', 'ZH', ]
    DEEPL_TO = ['BG', 'CS', 'DA', 'DE', 'EL', 'EN', 'EN-GB', 'EN-US', 'ES', 'ET', 'FI', 'FR', 'HU', 'ID', 'IT', 'JA',
    ...
    def _translate_finding(self, finding: Finding) -> FindingData:
        ...
        for key, value in finding.data.__dict__.items():  # TODO does this work with recursive fields?
    ...
    def _get_sysreptor_language_code(self, language_code) -> str:
        ...
        matched_lcs = [
            enabled_lc for enabled_lc in enabled_language_codes if enabled_lc.lower(
            ).startswith(language_code[:2].lower())
        ]
        if not matched_lcs:
